...
struct onnx_resolver_t {
    ...
    void (*op_Col2Im)(struct onnx_node_t *n);
    ...
    void (*op_GridSample)(struct onnx_node_t *n);
    ...
    void (*op_ImageDecoder)(struct onnx_node_t *n);
    ...
    void (*op_MelWeightMatrix)(struct onnx_node_t *n);
    ...
    void (*op_Optional)(struct onnx_node_t *n);
    void (*op_OptionalGetElement)(struct onnx_node_t *n);
    void (*op_OptionalHasElement)(struct onnx_node_t *n);
    ...
    void (*op_RegexFullMatch)(struct onnx_node_t *n);
    ...
    void (*op_STFT)(struct onnx_node_t *n);
    ...
    void (*op_StringConcat)(struct onnx_node_t *n);
    void (*op_StringNormalizer)(struct onnx_node_t *n);
    void (*op_StringSplit)(struct onnx_node_t *n);
    ...
    void (*op_ZipMap)(struct onnx_node_t *n);
};

struct onnx_context_t *onnx_context_alloc(const void *buf, size_t len, struct onnx_resolver_t **r,
                                          int rlen);
struct onnx_context_t *onnx_context_alloc_from_file(const char *filename,
                                                    struct onnx_resolver_t **r, int rlen);
...
struct onnx_tensor_t *onnx_tensor_alloc(const char *name, enum onnx_tensor_type_t type, int *dims,
                                        int ndim);
...
static inline void onnx_tensor_offset_to_indices(struct onnx_tensor_t *t, int offset,
                                                 int *indices) {
    int i;
...
static inline int onnx_tensor_reshape(struct onnx_tensor_t *y, int *dims, int ndim,
                                      enum onnx_tensor_type_t type) {
    if ((y->ndim != ndim) || (dims && (onnx_memcmp(y->dims, dims, sizeof(int) * y->ndim) != 0)) ||
        (y->type != type)) {
...
static inline int onnx_tensor_reshape_identity(struct onnx_tensor_t *y, struct onnx_tensor_t *x,
                                               enum onnx_tensor_type_t type) {
    if ((y->ndim != x->ndim) || (onnx_memcmp(y->dims, x->dims, sizeof(int) * y->ndim) != 0) ||
        (y->type != type)) {
...
static inline int onnx_tensor_reshape_multi_broadcast(struct onnx_tensor_t *y,
                                                      struct onnx_tensor_t *a,
                                                      struct onnx_tensor_t *b,
                                                      enum onnx_tensor_type_t type) {
    ...
    if ((y->type != type) || (y->ndim != ndim) ||
        (onnx_memcmp(y->dims, dims, sizeof(int) * ndim) != 0)) {
        onnx_tensor_reinit(y, type, dims, ndim);
    }
    return 1;
}
...
Onnx__GraphProto *onnx_attribute_read_graph(struct onnx_node_t *n, const char *name,
                                            Onnx__GraphProto *def);
Onnx__SparseTensorProto *onnx_attribute_read_sparse_tensor(struct onnx_node_t *n, const char *name,
